...
from models.ppi_model import PPIModel
    ...
    for batch_idx, (seq_data, pssm_data, dssp_data, local_data, label, msa_file, middle_fea) in enumerate(loader):
        ...
        with torch.no_grad():
            if torch.cuda.is_available():
                ...
                msa_var = torch.autograd.Variable(msa_file.cuda(async=True).float())
                middle_var = torch.autograd.Variable(middle_fea.cuda(async=True).float())
            else:
        ...
        # compute output
        # print('seq_var:'+str(seq_var.shape))
        # print('dssp_var:'+str(dssp_var.shape))
        # print('pssm_var:'+str(pssm_var.shape))
        # print('local_var:'+str(local_var.shape))
        # print('msa_var:'+str(msa_var.shape))
...
def train(class_tag,model, train_data_set, save, n_epochs=3,
    ...
    # Start log
    with open(os.path.join(save, 'PPI_results.csv'), 'w') as f:
        ...
        for epoch in range(n_epochs):
            ...
            if f_max > best_F:
                ...
                torch.save(model.state_dict(), os.path.join(save, 'PPI_model.dat'))
            else:
                count += 1
                if count>=5:
                    return None
...
def demo(train_data,save=None, train_num = 1, 
    ratio=None,window_size=3,splite_rate = 0.1, efficient=True,
              epochs=30, seed=None,pretrained_result=None):
    ...
    train_MSA_file = ['data_cache/{0}_MSA_features.npy'.format(key) for key in train_data]
    ...
    train_dataSet = data_generator.dataSet(window_size, train_sequences_file, train_pssm_file, train_dssp_file, train_label_file,
                                             all_list_file, train_MSA_file)
    # Models

    class_nums = 1
    model = PPIModel(class_nums,window_size,ratio)
    # model.cuda()
...
if __name__ == '__main__':
    ...
    demo(train_data,path_dir,0,ratio_list)
